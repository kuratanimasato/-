...
/**
 * スクリプトとスタイルをキューに追加します。
 */
function grandeur_shinohara_scripts()
{
	wp_enqueue_style('grandeur-shinohara-style', get_stylesheet_uri(), array(), _S_VERSION);
	wp_style_add_data('grandeur-shinohara-style', 'rtl', 'replace');

	wp_enqueue_script('grandeur-shinohara-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true);

	if (is_singular() && comments_open() && get_option('thread_comments')) {
		wp_enqueue_script('comment-reply');
	}
}
add_action('wp_enqueue_scripts', 'grandeur_shinohara_scripts');
...
function myTheme_enqueue_style_script()
{
	...
	wp_enqueue_script('gsap', 'https://cdnjs.cloudflare.com/ajax/libs/gsap/3.12.5/gsap.min.js', array(), null, true, 'defer', true);
	wp_enqueue_script('scrolltrigger', 'https://cdnjs.cloudflare.com/ajax/libs/gsap/3.12.5/ScrollTrigger.min.js', array('gsap'), null, true, 'defer', true);
	wp_enqueue_script('smooth-scroll', 'https://cdnjs.cloudflare.com/ajax/libs/smooth-scroll/16.1.3/smooth-scroll.polyfills.min.js', array('gsap'), null, true, 'defer', true);
	wp_enqueue_script('swiper', 'https://cdn.jsdelivr.net/npm/swiper@11/swiper-bundle.min.js', array(), null, true, 'defer', true);
...
{
	return get_theme_file_uri('/favicon.ico', $url);
}
/**************************************************
